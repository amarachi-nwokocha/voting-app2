...
export default function VotePage() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      console.warn("⚠️ Fetch timeout - setting loading to false")
      setLoading(false)
  ...
  const fetchContestants = async () => {
    try {
      ...
      setContestants(data || [])
      console.log("✅ Contestants loaded successfully:", data?.length || 0)
    } catch (error) {
      console.error("❌ Error fetching contestants:", error)
      setError(`Failed to load contestants: ${error instanceof Error ? error.message : 'Unknown error'}`)
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          <div className="text-xl mb-4">Loading contestants...</div>
          {error && (
